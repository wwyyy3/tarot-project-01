...
3. **扇形选牌** — 78 张牌以扇形展开，牌背为蓝色星盘图案。点击卡牌选中（最多 3 张），选中卡牌按选择顺序飞入顶部行展示，其余牌变暗淡化。支持重新洗牌、返回重选和窗口自适应重排。
4. **输入提问** — 输入占卜问题与 DeepSeek API Key。API Key 输入框支持密码/明文切换。底部提供"← 返回重选"按钮，可回到扇形阶段重新选牌。
...
- **扇形布局**：通过三角函数算法（`getFanTransform()`）计算弧形分布，每张牌沿弧线旋转以呈现立体感；选中牌通过 `getRowTransform()` 飞到顶部行
- **按选择顺序排列**：选中卡牌按照用户点击的先后顺序在顶部排列，而非按编号排序，更直观地展示用户的选牌意图
- **返回重选**：提问阶段提供"← 返回重选"按钮，可随时回到扇形选牌阶段调整选择
- **无滚动条全屏布局**：各阶段使用 `100dvh` 高度 + flex 自适应布局，窗口内完整展示，无滚动条
- **浏览器自动打开**：启动时检测操作系统，自动调用默认浏览器打开 `http://localhost:8080`
