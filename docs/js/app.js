...
askBtn.addEventListener('click', async function() {
    ...
    // 构建 prompt
    var prompt = '你是一位精通塔罗牌的占卜师。请根据用户抽到的卡牌和提出的问题，给出有深度、有启发性的解读。\n\n';
    prompt += '## 用户的问题：\n' + question + '\n\n';
    prompt += '## 抽到的卡牌（按抽取顺序）：\n';
    ...
    prompt += '请用温暖而专业的语言，给出详细的解读。请使用 Markdown 格式组织内容，包含清晰的小标题、列表和重点加粗。';
    ...
    try {
        var body = JSON.stringify({
            ...
            temperature: 0.8,
            messages: [
                { role: 'system', content: '你是塔罗牌占卜师，请给出有深度、有启发性的解读，并使用 Markdown 格式回答。' },
